...
> [HINT]1：需import  math模組，並使用math.pi。
> [HINT]2：輸出浮點數到小數點後第二位。
...
> [HINT]1：$歐式距離 = \sqrt{((x1-x2)^2+(y1-y2)^2)}$
> [HINT]2：兩座標的歐式距離，輸出到小數點後第4位
...
> [HINT]1：建議使用import math模組的math.pow及math.tan
> [HINT]2：正五邊形面積的公式： $Area=(5 * s^2)/(4 * tan(pi/5))$
> [HINT]3：輸出浮點數到小數點後第四位。
...
> [HINT]1：建議使用import math模組的math.pow及math.tan
> [HINT]2：正n邊形面積的公式如下：$Area = (n * s^2) / (4 * tan(pi/n))$
> [HINT]3：輸出浮點數到小數點後第四位
...
> [HINT]：轉換規則=十進位0-9的十六進位值為其本身，十進位10-15的十六進位值為A-F。
...
> [HINT]：計算平面上兩點距離的公式： $\sqrt{(x1-x2)^2 + (y1-y2)^2}\quad$
...
## Python 309 存款總額
請使用迴圈敘述撰寫一程式，> [HINT]使用者輸入金額（如10,000）、年收益率（如5.75），以及經過的月份數（如5），接著顯示每個月的存款總額。
> [HINT]：四捨五入，輸出浮點數到小數點後第二位。
舉例：
...
> [HINT]： $BMI = 體重(kg)/身高^2(m)$，輸出浮點數到小數點後第二位。 不需考慮男性或女性標準。
...
> [HINT]：費氏數列的某一項數字是其前兩項的和，而且第0項為0，第一項為1，表示方式如下：
...
> [HINT]1：平均溫度輸出到小數點後第二位。
> [HINT]2：最高溫度及最低溫度的輸出，如為31時，則輸出31，如為31.1時，則輸出31.1。
...
## Python 808 社會安全碼
請撰寫一程式，> [HINT]使用者輸入一個社會安全碼SSN，格式為ddd-dd-dddd，d表示數字。若格式完全符合（正確的SSN）則顯示【Valid SSN】，否則顯示【Invalid SSN】
